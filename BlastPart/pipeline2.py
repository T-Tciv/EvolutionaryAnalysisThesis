...
while current_procs_count > 0:
    break # !!!!!!!!!!!!!!!!!!!!!!!!!!! этот цикл не используется
    removed_count = 0
    for i in done_numbers:
        procs.pop(i - removed_count)
        commands.pop(i - removed_count)
        output_list.pop(i - removed_count)
    ...
    if (error_counter > 20):
        for p in procs:
            p.terminate()
        current_procs_count = 0
        break

    #print("procs list size:", len( procs ), "current procs count:", current_procs_count, "errors count", error_counter)

    for i in range(current_procs_count):
        time.sleep(30)
        p = procs[i]
        if (p.poll() is None):
            if (time.time() - start_time > 25200):
                p.terminate()
            continue
        
        done_numbers.append(i)
        print("Завершился запуск: {}".format(p.args))
        print("Идёт обработка выходных данных")
        
        output, err = p.communicate()
        if (err):
            new_output_list.append(output_list[i])
            current_command = commands[i]
            new_commands.append(current_command)
            print()
            print(time.strftime("%H:%M:%S", time.localtime()))
            print("Запуск упал с ошибкой, перезапускаем: ", current_command)
            print(output_list[i])
            print(err)
            new_procs.append(subprocess.Popen( current_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE))
            ...
            print()
            print(time.strftime("%H:%M:%S", time.localtime()))
            print("Запуск успешно завершился: ", current_command)
            out_file = open(output_list[i] + "_stdout.txt", "wb") 
            out_file.write(output) 
            out_file.close()

    current_procs_count = current_procs_count - len( done_numbers )
